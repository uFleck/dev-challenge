...
- Primeiro, eu criei um componente blazor, e coloquei dentro dele o código para fazer a requisição ao back-end.
- Formatei os dados que aparecerem.
- Validei a pesquisa, tanto no front-end quanto no back-end.
- Criei a classe DomainLookup, com a função Request, onde se encontra o código que repetia no DomainController, para diminuir a complexidade ciclomática, assim como aumentar a legibilidade do código.
- Criei uma DTO, para retornar apenas os valores necessários para o usuário.
- Dei uma enfeitadinha no front-end.
- Adicionei Swagger ao projeto.
- Sobre os testes: Infelizmente não consegui progredir muito. Este foi meu primeiro contato com testes unitários e mock, portanto, não consegui aprender o suficiente no tempo que tive para aplicar na situação deste projeto.
